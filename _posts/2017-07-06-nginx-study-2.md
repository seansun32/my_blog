...
int main(){
    ngx_queue_t queue_g;
    ngx_queue_t *q, *prev, *next, tail; 

    int i=0;
    ngx_queue_node_t *tmp_node=NULL;

    //init queue
    ngx_queue_init(&queue_g);

    
    //set value to queue node, and add node to queue
    for(i=0;i<10;i++){
        tmp_node=calloc(1,sizeof(ngx_queue_node_t));
        tmp_node->val=i;
        
        if(i%2==0){
            ngx_queue_insert_head(&queue_g,&tmp_node->queue);
        }else{
            ngx_queue_insert_tail(&queue_g,&tmp_node->queue); 
        }
        tmp_node=NULL;
    }

    q=ngx_queue_head(&queue_g);
    if(q==ngx_queue_last(&queue_g)){
        printf("no node\n");
        return -1;
    }
    ...
    for(q=ngx_queue_head(&queue_g);q!=ngx_queue_sentinel(&queue_g);q=next){
        //prev=ngx_queue_prev(q);
        next=ngx_queue_next(q);
        tmp_node=ngx_queue_data(q,ngx_queue_node_t,queue);
        if(tmp_node->val==5){
            ngx_queue_remove(&tmp_node->queue);
            free(tmp_node);
        }
    }
    ...
    for(q=ngx_queue_head(&queue_g);q!=ngx_queue_sentinel(&queue_g);q=next){
        ...
        tmp_node=ngx_queue_data(q,ngx_queue_node_t,queue);
        if(tmp_node->val==4){
            ngx_queue_split(&queue_g,&tmp_node->queue,&tail);//tail is the new sentinel node for new splited list 
            break;
        }
    }
    for(q=ngx_queue_head(&queue_g);q!=ngx_queue_sentinel(&queue_g);q=next){
    ...
    for(q=ngx_queue_head(&tail);q!=ngx_queue_sentinel(&tail);q=next){
        next=ngx_queue_next(q);
        tmp_node=ngx_queue_data(q,ngx_queue_node_t,queue);
